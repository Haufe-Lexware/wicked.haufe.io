...
```
$ export WICKED_VERSION=1.0.0-rc.1 # Possibly adapt to the latest version
...
`postgres.deployPostgres` | - | `true` | Decides whether a dedicated PostgreSQL instance for Kong is deployed as a part of this installation. This is good for test and development deployment, but if you want to use a different PostgreSQL instance, specify `false` here. **This is recommended for production deployments.**
...
`postgres.pgSsl` | - | `false` | Set to `true` to set the Postgres SSL mode to "require". This is especially needed when using a managed Postgres service such as Azure Database for Postgres, which requires this option and does not let the client negotiate.
